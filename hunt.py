...
def tool_httpx():
    ...
    # mode pilih sumber list (1=auto bbp/<target>/subs.txt, 2=custom path, 3=root target langsung)
    choice_map = {
        "1": ("Use ~/bug-hunting/bbp/<target>/subs.txt", "auto"),
        "2": ("Custom path to list (-l)", "custom"),
        "3": ("Root target only (fallback -u)", "root"),
    }
    src_mode = ask_menu("Pick source", choice_map, multi=False, allow_blank=False)
    ...
    if src_mode == "auto":
        ...
        temp_dir  = os.path.join(BBP_BASE, tclean)
        ensure_dir(temp_dir)
        input_list = os.path.join(temp_dir, "subs.txt")
        root_target = base_name
    elif src_mode == "custom":
        input_list = ask("Custom list path")
        root_target = ask("Root domain (example.com) for fallback -u")
        temp_dir  = os.path.join(BBP_BASE, sanitize_target_for_dir(root_target))
        ensure_dir(temp_dir)
    else:  # root
        root_target = ask("Root domain (example.com)")
        tclean    = sanitize_target_for_dir(root_target)
        temp_dir  = os.path.join(BBP_BASE, tclean)
        ensure_dir(temp_dir)
        input_list = None

    out_file = os.path.join(temp_dir, "httpx.txt")
    mode, value = pick_url_or_list_for_tool(input_list, root_target, threads=threads, mc=mc, try_both=True)

    if mode == "list":
        cmd = ["httpx", "-l", value, "-mc", mc, "-t", threads, "-silent", "-o", out_file]
    else:
        cmd = ["httpx", "-u", value, "-mc", mc, "-t", threads, "-silent", "-o", out_file]
...
def tool_nuclei():
    ...
    choice_map = {
        ...
        "2": ("Scan from httpx/gau/subfinder with fallback", "smart"),
        "3": ("Scan single URL (-u https://..)", "single"),
    }
    ...
    base_cmd = ["nuclei", "-c", conc]
    if len(sev_list) > 0:
        base_cmd += ["-severity", ",".join(sev_list)]

    if scan_mode == "list":
        live_list = ask("Path to list for nuclei -l")
        out_file  = os.path.join(os.path.dirname(os.path.abspath(live_list)), "nuclei.txt")
        cmd = base_cmd + ["-l", live_list, "-o", out_file]
        ...
        # prefer httpx.txt → gau.txt → subs.txt
        httpx_list = os.path.join(temp_dir, "httpx.txt")
        gau_list   = os.path.join(temp_dir, "gau.txt")
        subs_list  = os.path.join(temp_dir, "subs.txt")

        candidate = httpx_list if has_nonempty_file(httpx_list) else (gau_list if has_nonempty_file(gau_list) else (subs_list if has_nonempty_file(subs_list) else None))
        mode, value = pick_url_or_list_for_tool(candidate, root_target, threads=conc, mc="200", try_both=True)

        out_file = os.path.join(temp_dir, "nuclei.txt")
        if mode == "list":
            cmd = base_cmd + ["-l", value, "-o", out_file]
        else:
            cmd = base_cmd + ["-u", value, "-o", out_file]

    else:
        single_url = ask("Single URL (https://target.com)")
        tclean   = sanitize_target_for_dir(single_url)
        temp_dir = os.path.join(BBP_BASE, tclean); ensure_dir(temp_dir)
        out_file = os.path.join(temp_dir, "nuclei.txt")
        cmd = base_cmd + ["-u", single_url, "-o", out_file]
...
def tool_xray():
    ...
    choice_map = {
        "1":("Single or root with fallback","smart"),
        "2":("Multi from file (--url-file)","list"),
    }
    mode_choice = ask_menu("Scan mode", choice_map, multi=False, allow_blank=False)
    ...
    if mode_choice == "list":
        ...
        cmd = [XRAY_WRAPPER_BIN, "webscan", "--url-file", url_file, "--json-output", out_json]
    else:
        root_target = ask("Root domain or full URL (example.com / https://example.com)")
        # kalau user kasih full URL langsung aja
        if root_target.startswith("http://") or root_target.startswith("https://"):
            tclean = sanitize_target_for_dir(root_target)
            temp_dir = os.path.join(BBP_BASE, tclean); ensure_dir(temp_dir)
            out_json = os.path.join(temp_dir, "xray.json")
            cmd = [XRAY_WRAPPER_BIN, "webscan", "--url", root_target, "--json-output", out_json]
        else:
            # root → cari list/httpx, kalau kosong fallback ke -u https://root (lalu http)
            tclean = sanitize_target_for_dir(root_target)
            temp_dir = os.path.join(BBP_BASE, tclean); ensure_dir(temp_dir)
            out_json = os.path.join(temp_dir, "xray.json")

            httpx_file = os.path.join(temp_dir, "httpx.txt")
            mode, value = pick_url_or_list_for_tool(httpx_file, root_target, threads="50", mc="200", try_both=True)
            if mode == "list":
                cmd = [XRAY_WRAPPER_BIN, "webscan", "--url-file", value, "--json-output", out_json]
            else:
                cmd = [XRAY_WRAPPER_BIN, "webscan", "--url", value, "--json-output", out_json]
...
def has_nonempty_file(path):
    try:
        return os.path.isfile(path) and any(line.strip() for line in open(path, "r", encoding="utf-8", errors="ignore"))
    except Exception:
        return False

def try_httpx_probe(url, out_path, threads="50", mc="200"):
    cmd = ["httpx", "-u", url, "-mc", mc, "-t", threads, "-silent", "-o", out_path]
    run_cmd(cmd)
    return has_nonempty_file(out_path)

def pick_url_or_list_for_tool(possible_list_path, root_target, threads="50", mc="200", try_both=True):
    """
    Return tuple: (mode, value)
      - ("list", <path>)  -> pakai -l
      - ("url",  <url>)   -> pakai -u
    Logic:
      - kalau file list ada & berisi -> ("list", path)
      - else probe https://target pakai httpx; kalau hidup -> ("url","https://target")
      - kalau nggak & try_both True -> probe http://target; kalau hidup -> ("url","http://target")
      - kalau semua gagal -> ("url","https://target") (tetap pakai root agar tool lain jalan)
    """
    if possible_list_path and has_nonempty_file(possible_list_path):
        return ("list", possible_list_path)

    # buat workspace sementara untuk probe
    tclean = sanitize_target_for_dir(root_target)
    temp_dir = os.path.join(BBP_BASE, tclean)
    ensure_dir(temp_dir)
    probe_out = os.path.join(temp_dir, "_probe_httpx.txt")

    if try_httpx_probe(f"https://{root_target}", probe_out, threads=threads, mc=mc):
        return ("url", f"https://{root_target}")
    if try_both and try_httpx_probe(f"http://{root_target}", probe_out, threads=threads, mc=mc):
        return ("url", f"http://{root_target}")

    # fallback terakhir: tetap kembalikan https (biar tool yang butuh -u tetap jalan)
    return ("url", f"https://{root_target}")
...
def run_cli():
    banner_main()
    print("CLI mode aktif ✅ (ketik pilihan di menu)")
...
def run_gui():
    ...
    buttons = [
        ("Run RECON Menu", "python3 -c 'import hunt; hunt.recon_menu()'"),
        ("FullPower", "python3 -c 'import hunt; hunt.run_fullpower()'"),
        ("Attack Focus", "python3 -c 'import hunt; hunt.run_attack_focus()'"),
        ("Custom Chain", "python3 -c 'import hunt; hunt.run_custom_chain()'"),
        ("Exit", "exit")
    ]
    for label, cmd in buttons:
        tk.Button(root, text=label, width=30, height=2, bg="#333", fg="white",
                  command=(lambda c=cmd: run_shell(c) if c != "exit" else root.destroy())
                 ).pack(pady=5)

    root.mainloop()

if __name__ == "__main__":
    # Default GUI; --cli untuk CLI
    if "--cli" in sys.argv:
        # Submode opsional: python3 hunt.py --cli fullpower|attack|recon|custom
